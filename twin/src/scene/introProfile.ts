...
export function buildIntroProfile(
  ...
  // 乘性调制 0.78~1.0（直线走满巡航，最紧段 −22%）。第 25 轮把下限从 0.65
  // 提到 0.78：速度波动更平缓，过弯减速不再"一顿一顿"，观感更丝滑。
  ...
  for (let i = 0; i < N; i++) {
    const curvFactor = 0.78 + 0.22 * (1 - smooth01(kSm[i] / kRef))
...
/** 巡航段 boost 表：直线保持高速、弯道柔和减速，节奏连续、幅度克制（第 25 轮收敛）。
 *  与曲率温和调制(0.78~1.0)相乘后峰谷比 ≈ 2.4×，变速点更少、更缓，
 *  不再 0.55→1.50 陡冲——开场的"直道快、弯道缓"保留，但观感是连续滑行而非顿挫。 */
export const BOOST_TABLE: ReadonlyArray<readonly [number, number]> = [
  [0.00, 0.78],  // 高空起始：平稳起步
  [0.10, 1.00],  // 俯冲直道提速
  [0.22, 1.10],  // 冲底高速
  [0.32, 0.82],  // 低位弯道柔和减速
  [0.45, 1.18],  // 全景大直线（可控峰值，不再 1.40）
  [0.58, 1.22],  // 直推 T03（主峰值）
  [0.68, 0.88],  // T03 转头减速
  [0.78, 1.12],  // 对角线穿场
  [0.86, 0.90],  // 转向 T07 减速
  [0.94, 0.84],  // 接近段
  [1.00, 0.78],  // 交棒收尾环绕（近悬停）
]
