...
//   B2 站位表：58.90（原误 59.00）、叶尖弦长 1.417 m（原误 0.900）
//   B3 塔基直径：y=0 处 r=3.0 m（6.0 m 铭牌直径），仅过渡段略放大
//   B4 删除无依据"后掠"与机舱鳍片（NREL 5MW 均不存在）
//   B5 死代码清除
//   B6 周向采样 26 → 34，近景剖面不再呈十三边形
// 诚实边界：翼型截面为"DU/NACA64 家族"的解析近似（四位数厚度 + 线性
// 降弯度中线），不是逐点 DU21-40/NACA64-A16 坐标表 —— 属【示意几何】，
// 只用于可视化，禁止作为气动/结构计算输入。
// ============================================================

/** NREL 5MW 叶片站位表（OSTI 1095962 公开表）：r(m) 弦长(m) 扭角(deg) 相对厚度 */
export const STATIONS: [number, number, number, number][] = [
  ...
  [58.90, 2.086, 0.370, 0.17], // B2 修正：58.90（原误 59.00）
  [60.50, 1.710, 0.106, 0.15],
  [61.50, 1.417, 0.000, 0.18], // B2 修正：叶尖弦长 1.417 m；NACA64-A16 尾部厚度 ~18%
]
